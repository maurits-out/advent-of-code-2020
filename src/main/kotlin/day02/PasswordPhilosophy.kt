...
class PasswordPhilosophy(private val input: List<String>) {
    ...
    fun part1() = input.count { isValidPasswordPart1(it) }

    fun part2() = input.count { isValidPasswordPart2(it) }

    private fun isValidPasswordPart1(line: String) =
        isValidPassword(line) { password, char, min, max ->
            password.count { it == char } in min..max
        }

    private fun isValidPasswordPart2(line: String) =
        isValidPassword(line) { password, char, pos1, pos2 ->
